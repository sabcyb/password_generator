...
function password_generator (){
    ...
    for (let i=0; i<minimumLen; i++){
        ...
        if(charDecider === 1){
            ...
            password1 += smallChar[Math.ceil(Math.random() * smallChar.length)];
        } else if (charDecider === 3){
            password1 += number[Math.ceil(Math.random() * number.length)];
        } else{
            password1 += special[Math.ceil(Math.random() * special.length)];
        }
    }
    return password1;
};
...
document.querySelector("#btn").addEventListener("click",function(){
    let pass_1 = password_generator();
    let pass_2 = password_generator();
